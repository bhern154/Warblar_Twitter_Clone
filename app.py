...
from flask import Flask, render_template, request, flash, redirect, session, g, abort, jsonify, Response
...
def messages_show(message_id):
    ...
    if msg is None:
        # Return a 404 error if the message does not exist
        abort(404)
...
def homepage():
    ...
    """
    if g.user:
        # ids from users the current user is following
        following_ids = [user.id for user in g.user.following]
        # append current user's id
